  ...
  db("projects")
    ...
    .then(project => {
      if (!project) {
        return res
          .status(404)
          .send(`No entry found in "projects" with id of ${id}.`);
      }
  ...
  db("palettes")
    ...
    .then(palette => {
      if (!palette) {
        return res
          .status(404)
          .send(`No entry found in "palettes" with id of ${id}.`);
      }
  ...
  db("palettes")
    ...
    .then(palette => {
      if (!palette) {
        return res
          .status(404)
          .send(`No entry found in "palettes" with id of ${id} to update.`);
      }
  ...
  for (let param of required) {
    if (!projectData[param]) {
      return res
        .status(422)
        .send(
          `Expected format: { name: <String>, id: <number>}. You are missing the ${param} parameter.`
        );
    }
  }
  ...
  db("projects")
    ...
    .then(palette => {
      if (!palette) {
        return res
          .status(404)
          .send(`No entry found in "projects" with id of ${id} to update.`);
      }
      ...
      db("projects")
        ...
        .update(projectData)
        .then(() =>
          res.status(200).send(`ProjectData ${id} successfully updated.`)
        )
        .catch(() => res.sendStatus(500));
    })
    .catch(() => res.sendStatus(500));
  ...
  db("palettes")
    ...
    .then(palette => {
      if (!palette) {
        return res
          .status(200)
          .send(`No entry found in "palettes" with id of ${id} to delete.`);
      }
  ...
  db("projects")
    ...
    .then(project => {
      if (!project) {
        return res
          .status(200)
          .send(`No entry found in "projects" with id of ${id} to delete.`);
      }
